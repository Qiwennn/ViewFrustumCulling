...
void CameraSceneNode::do_CALCULATE_TRANSFORMATIONS(Events::Event *pEvt)
{
	...
	const Matrix4x4 &mref_worldTransform = m_worldTransform;

	const Vector3 pos(mref_worldTransform.m[0][3], mref_worldTransform.m[1][3], mref_worldTransform.m[2][3]);
	const Vector3 n  (mref_worldTransform.m[0][2], mref_worldTransform.m[1][2], mref_worldTransform.m[2][2]);
	const Vector3 up (mref_worldTransform.m[0][1], mref_worldTransform.m[1][1], mref_worldTransform.m[2][1]);
	const Vector3 target = pos + n;
	...
	const Vector3 pos2(m_worldTransform2.m[0][3], m_worldTransform2.m[1][3], m_worldTransform2.m[2][3]);
	const Vector3 n2  (m_worldTransform2.m[0][2], m_worldTransform2.m[1][2], m_worldTransform2.m[2][2]);
	const Vector3 up2 (m_worldTransform2.m[0][1], m_worldTransform2.m[1][1], m_worldTransform2.m[2][1]);
	const Vector3 target2 = pos2 + n2;
    ...
    {
        //ios portrait view
        static const PrimitiveTypes::Float32 factor = 0.5f;
        verticalFov *= factor;
    }
	...
	const PrimitiveTypes::Float32 verticalFov2 = 0.318f * PrimitiveTypes::Constants::c_Pi_F32;
	const Matrix4x4 m_viewToProjectedTransform2 = CameraOps::CreateProjectionMatrix(verticalFov2, aspect, m_near, m_far);
	// ***
