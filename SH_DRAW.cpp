    ...
    if (pMeshCaller->m_performBoundingVolumeCulling)
    {
		...
		for (int iInst = 0; iInst < pMeshCaller->m_instances.m_size; ++iInst)
		{
            ...
            if (curSN)
            {
				...
				const Matrix4x4 worldMatrix = curSN->m_worldTransform;

				for (const Vector3 &vertex : pMeshCaller->m_AABBPoints)
				{
					const Vector3 vertexTransformed = worldMatrix * vertex;
					int insideCounter = 0;

					for (const Plane &plane : pcam->m_frustumPlanes)
					{
				...
				for (int i = 0; i < 4; ++i)
				{
					...
					const Matrix4x4 m = worldMatrix * pMeshCaller->m_AABB[i];
